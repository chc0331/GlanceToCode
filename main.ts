...
function convertRectangleToBox(rect: RectangleNode): string {
  ...
  if (rect.fills && rect.fills !== figma.mixed) {
    const fills = rect.fills as ReadonlyArray<Paint>;
    const solid = fills.find((p): p is SolidPaint => p.type === 'SOLID');
    if (solid) {
...
function generateCode(): string {
  ...
  const rectangles = selection.filter((n): n is RectangleNode => n.type === 'RECTANGLE');
